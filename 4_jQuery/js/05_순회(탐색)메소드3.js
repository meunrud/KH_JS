...
const style3 = { border: '2px solid skyblue', color: 'skyblue' };
const style4 = { border: '2px solid lightgreen', color: 'lightgreen' };
const style5 = { border: '2px solid purple', color: 'purple' };

//* siblings() : 선택한 요소의 형제 요소를 모두 선택
//h2 태그의 형제 요소에 스타일로 style1 설정
$('h2').siblings().css(style1);

// *sibling("매개변수(css선택자")) : 선택한 요소의 형제 요소 중에서 매개변수로 작성된 요소만 선택
...
//* nextAll() : 선택된 요소의 다음 형제 요소를 선택하는 메소드
...
//span태그부터 h3 태그까지 폰트 크기를 30px로 변경
$('p').nextUntil('p').css('fontSize', '30px');

//* is("css선택자") : 지정된 범위 내에 매개변수로 지정된 선택자와 일치하는 요소가 존재하는지 확인하는 메소드
...
//h2 태그를 클릭 했을 때 클릭한 요소의 이전 요소의 배경색을 red 글자색 white

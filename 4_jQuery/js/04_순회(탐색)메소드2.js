$(document).ready(function () {
    //id가 test인 요소의 테두리를 2px 빨간 실선 글자 파란색으로 변경
...
const style3 = { border: '2px solid pink', color: 'pink' };
...
//-----------------------------------
//* children() : 선택된 요소 바로 하위요소(자식)를 선택하는 메소드.
//* 클래스가 wrap인 요소의 자식 스타일을 style1으로 설정
$('.wrap').children().css(style1);

//ul, p 태그의 스타일을 style2 로 설정
//$('ul, p').css(style2);
//$('.type').children().css(style2);
$('.wrap').children().children().css(style2);

//*children("매개변수(css선택자)") : 선택된 자식 요소 중에서 선택자와 일치하는 자식만 선택
...
//* find("매개변수(css선택자)") : 선택된 요소의 후손 중 매개변수로 작성된 선택자와 일치하는 모든 요소 선택
